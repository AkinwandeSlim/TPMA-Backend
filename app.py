...
#     """Parse HH:MM or ISO timestamp to HH:MM:SS."""
#     if not time_str:
#         return ""
#     try:
#         # Handle HH:MM
...
#             return time_str
#         # Handle ISO timestamp (e.g., 2025-04-07T09:00:00Z)
#         elif "T" in time_str:
...
def require_3auth(roles):
    token = request.headers.get("Authorization", "").replace("Bearer ", "")
    if not token:
        return None, {"error": "Token is missing", "status": 401}
    try:
        decoded = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
...
def get_trainee_lesson_plans(trainee_id):
    if request.method == "OPTIONS":
        return jsonify({"status": "ok"}), 200
    ...
    try:
        ...
        for lp in lesson_plans:
            ...
            normalized_plan = {
                ...
                "subject": str(lp.get("subject", "")),
                "class": str(lp.get("class", "")),
                ...
                "objectives": str(lp.get("objectives", "")),
                "activities": str(lp.get("activities", "")),
                "resources": str(lp.get("resources", "")),
                "createdAt": normalize_datetime(lp.get("createdAt")),  # Normalize to ISO format
                "status": str(lp.get("status", "PENDING")).upper(),
                "aiGenerated": bool(lp.get("aiGenerated", False)),
                "traineeName": str(lp.get("traineeName", "")),
                "supervisorName": str(lp.get("supervisorName", "")),
                "schoolName": str(lp.get("schoolName", "")),
        ...
        if subject:
            filtered_plans = [lp for lp in filtered_plans if lp.get("subject", "") == subject]
        if status:
            filtered_plans = [lp for lp in filtered_plans if lp.get("status", "") == status.upper()]
