...
#     users_data = users
#      # Inside create_tp_assignment, before creating new_assignment
#     # existing_assignment = next((a for a in users_data.get('tp_assignments', []) if a['traineeId'] == data['traineeId']), None)
#     # if existing_assignment:
#     #     logger.error(f"Trainee {data['traineeId']} already has a TP assignment")
#     #     return jsonify({"error": "Trainee already has a TP assignment"}), 400
...
# # @app.route('/api/admin/tp_assignments/<assignment_id>', methods=['DELETE', 'OPTIONS'])
# # @_require_auth(['admin'])
# # def delete_tp_assignment(decoded, assignment_id):
# #     if request.method == 'OPTIONS':
# #         response = jsonify({"status": "ok"})
# #         response.headers['Access-Control-Allow-Origin'] = 'http://localhost:3000'
# #         response.headers['Access-Control-Allow-Credentials'] = 'true'
...
# #     try:
# #         users_data = load_users()
# #         assignment = next((a for a in users_data.get('tp_assignments', []) if a['id'] == assignment_id), None)
# #         if not assignment:
# #             logger.error(f"TP assignment not found: {assignment_id}")
# #             response = jsonify({"error": "TP assignment not found"})
# #             response.headers['Access-Control-Allow-Origin'] = 'http://localhost:3000'
# #             response.headers['Access-Control-Allow-Credentials'] = 'true'
# #             return response, 404
...
# #         # Fetch names for notifications
# #         trainee = next((t for t in users_data.get('teacherTrainee', []) if t['id'] == assignment['traineeId']), None)
# #         supervisor = next((s for s in users_data.get('supervisor', []) if s['id'] == assignment['supervisorId']), None)
# #         school = next((s for s in users_data.get('schools', []) if s['id'] == assignment['schoolId']), None)
# #         trainee_name = f"{trainee.get('name', '')} {trainee.get('surname', '')}".strip() if trainee else assignment['traineeId']
# #         supervisor_name = f"{supervisor.get('name', '')} {supervisor.get('surname', '')}".strip() if supervisor else assignment['supervisorId']
# #         school_name = school.get('name', '') if school else 'Unknown'

# #         with lock:
# #             # Remove assignment
# #             users_data['tp_assignments'] = [a for a in users_data.get('tp_assignments', []) if a['id'] != assignment_id]
...
# #                 'type': 'ASSIGNMENT',
# #                 'priority': 'HIGH',
# #                 'message': f'Your TP assignment at {school_name} has been removed.',
# #                 'created_at': now,
# #                 'read_status': False
# #             }
# #             supervisor_notification = {
...
# #                 'type': 'ASSIGNMENT',
# #                 'priority': 'MEDIUM',
# #                 'message': f'Your supervision assignment for {trainee_name} at {school_name} has been removed.',
# #                 'created_at': now,
# #                 'read_status': False
# #             }
# #             notifications.extend([trainee_notification, supervisor_notification])
...
# #         logger.info(f"Deleted TP assignment {assignment_id}")
# #         response = jsonify({'message': 'TP assignment deleted successfully'})
# #         response.headers['Access-Control-Allow-Origin'] = 'http://localhost:3000'
# #         response.headers['Access-Control-Allow-Credentials'] = 'true'
# #         return response, 200

# #     except Exception as e:
# #         logger.error(f"Error deleting TP assignment {assignment_id}: {str(e)}")
# #         response = jsonify({"error": "Failed to delete TP assignment", "details": str(e)})
# #         response.headers['Access-Control-Allow-Origin'] = 'http://localhost:3000'
# #         response.headers['Access-Control-Allow-Credentials'] = 'true'
# #         return response, 500
